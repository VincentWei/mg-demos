...
public:
    ...
    void list(void) {
        _MG_PRINTF ("All registered activities (total %lu):\n", (unsigned long)m_activities.size());
        for (std::vector<ActivityInfo *>::const_iterator i=m_activities.begin(); i!=m_activities.end(); ++i) {
            _MG_PRINTF ("  %s (%p)\n", (*i)->name, (*i)->create);
        }
    }
    ...
    void setGlobalHosting (HWND hwnd = HWND_DESKTOP) {
        m_global_hosting = hwnd;
    }

    HWND getGlobalHosting () {
        return m_global_hosting;
    }
...
private:
    ...
    /* the global hosting window for all activities */
    HWND m_global_hosting;
};

#define SET_GLOBAL_HOSTING(hwnd) \
    ActivityFactory::singleton()->setGlobalHosting(hwnd)

#define GET_GLOBAL_HOSTING \
    ActivityFactory::singleton()->getGlobalHosting()
