...
#include "MUCSettingsActivity.hh"
#include "MenuListPiece.hh"

#include "SysConfService.hh"

REGISTER_ACTIVITY(MUCSettingsActivity);
...
{
    const UPLOAD_OPTIONS* upload_opts = SYSCFGSERVICE->getMUCOptions (true);
...
static LRESULT mymain_onSwitchStateChanged (mMainWnd* self, UINT msg, DWORD wparam, DWORD lparam)
{
    ...
    if (row == (IDE_AUTO_DELETE - IDE_MIN)) {
        SYSCFGSERVICE->setMUCOptionAutoDel (event_id == NCSN_SLIDESWITCHPIECE_ON);
    }
...
static LRESULT mymain_onReady (mMainWnd* self, UINT msg, DWORD wparam, DWORD lparam)
{
    if (wparam == ACTIVITY_POPPED) {
        ...
        if (filter > 0) {
            if (SYSCFGSERVICE->setMUCOptionFilter (filter)) {
...
{
    _MG_PRINTF ("MUCSettingsActivity: mymain_onClose called.\n");
...
MUCSettingsActivity::~MUCSettingsActivity()
{
}
