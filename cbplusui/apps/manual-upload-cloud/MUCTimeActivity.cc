...
#include "MUCTimeActivity.hh"
#include "MenuListPiece.hh"

REGISTER_ACTIVITY(MUCTimeActivity);

extern MENU_ENTRY g_menu_entries_muc_time [];
extern int g_nr_entries_muc_time;

static BOOL mymain_onCommand (mMainWnd* self, int id, int nc, HWND hCtrl)
{
    if (id == IDC_CONTENT && nc < g_nr_entries_muc_time
            && g_menu_entries_muc_time[nc].type == TYPE_MENU_ENTRY_CHECKABLE) {
        ACTIVITYSTACK->pop (nc);
    }
...
static BOOL mymain_onCreate (mMainWnd* self, DWORD dwAddData)
{
    ...
    if (ctrl_ctnr) {
        ...
        createMenuListView ((mContainerCtrl*)ctrl_ctnr, &rc, g_menu_entries_muc_time, g_nr_entries_muc_time);
    }
...
{
    _MG_PRINTF ("MUCTimeActivity: mymain_onClose called.\n");
...
static BOOL mymain_onCreated (struct _NCS_CREATE_NOTIFY_INFO* info, mComponent* self, DWORD special_id)
{
    ...
    _c(piece_title)->setProperty (piece_title, NCSP_LABELPIECE_LABEL, (DWORD)_("Upload Period"));
...
MUCTimeActivity::MUCTimeActivity() : NCSActivity (&mymain_templ)
{
    m_style = STYLE_PUSH;
}

MUCTimeActivity::~MUCTimeActivity()
{
}
