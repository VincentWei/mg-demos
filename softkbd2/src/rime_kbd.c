...
*/
/*
** rime_kbd.c: This file include rime soft keyboard code.
**
...
// translate 和predict一样的话，会有判断逻辑出错
static md_ime_t rime_ime = {
    100,
    "rime",
    NULL,
    // cb_rime_match_keystrokes,
    cb_rime_translate_word,
    cb_rime_predict_word,
    cb_rime_init};

static md_key_t rime_key_pads[] = {
...
static md_key_t *get_rime_key(key_window_t *kw, POINT p)
{
    int i;
...
int rime_proceed_msg(key_board_t *key_board, HWND hwnd,
...
static int init_rime_view_window(HWND hWnd, view_window_t *vw)
{
...
static void destroy_rime_view_window(view_window_t *vw)
{
...
static int init_rime_stroke_window(HWND hWnd, stroke_window_t *sw)
{
...
static void destroy_rime_stroke_window(stroke_window_t *sw)
{
    if (sw->stroke_font)
        DestroyLogFont(sw->stroke_font);
}

static int init_rime_key_window(HWND hWnd, key_window_t *kw)
{
    int i;
    md_key_t *key;
    ...
    kw->key = rime_key_pads;
    kw->key_num = TABLESIZE(rime_key_pads);

    kw->get_key = get_rime_key;
    ...
    if (get_kbd_bitmap(HDC_SCREEN,
                       ...
                       CHAR_KEY_MASK))
    {
        _MY_PRINTF("Fail to get rime char key mask bitmap.\n");
        return -1;
    }
    ...
    if (get_kbd_bitmap(HDC_SCREEN,
                       ...
                       FUNC_KEY_MASK))
    {
        _MY_PRINTF("Fail to get rime func key mask bitmap.\n");
        return -1;
    }
...
static void destroy_rime_key_window(key_window_t *kw)
{
    if (kw->data)
    {
...
int init_rime_keyboard(HWND hWnd, key_board_t *kb)
{
    ...
    if (init_rime_view_window(hWnd, kb->view_window))
    {
    ...
    if (init_rime_stroke_window(hWnd, kb->stroke_window))
    {
    ...
    if (init_rime_key_window(hWnd, kb->key_window))
    {
    ...
    if (get_kbd_bitmap(HDC_SCREEN, (PBITMAP)(kb->data), RIME_KBD_BKG))
    {
        _MY_PRINTF("Fail to get rime kbd bkgnd bitmap.\n");
        return -1;
    }
    ...
    kb->proceed_msg = rime_proceed_msg;
...
{
    /*view window*/
    destroy_rime_view_window(kb->view_window);
    free(kb->view_window);

    /*stroke window*/
    destroy_rime_stroke_window(kb->stroke_window);
    free(kb->stroke_window);

    /*ky window*/
    destroy_rime_key_window(kb->key_window);
    free(kb->key_window);
